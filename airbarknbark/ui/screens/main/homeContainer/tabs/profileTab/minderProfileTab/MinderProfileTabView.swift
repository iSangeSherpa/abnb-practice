...
class MinderProfileTabView : BaseUIView{
    ...
    let profileIncompleteLabel = Caption(label: .ProfileTab.profileVerificationStatus.REVIEW_REQUIRED, color: .error)
    ...
    let changeUserType =  PrimaryButton(label: "Be a Finder", height: 42, cornerRadius: 4).apply{ (it:UIButton) in
        it.setImage(UIImage(named: "ic_location_person")?.scaledDown(into: .init(width: 14, height: 14)).withTintColor(.onBackground), for: .normal)
    ...
    lazy var pastWorksActionItem = ProfileActionItem(label:"History")
    lazy var yourReviewsActionItem = ProfileActionItem(label:"Your Ratings")
    lazy var deleteAccountActionItem = PrimaryButton(label: "Delete Account").apply { it in
        it.setTitleColor(UIColor.red, for: .normal)
        it.setTitleColor(UIColor.red, for: .highlighted)
    ...
    lazy var menuItems = stack(
        ...
        pastWorksActionItem,
        yourReviewsActionItem,
        ProfileActionItem(label:"Preferred Currency", trailingView: Caption(label: "AUD", color: .primary, font: .poppinsSemibold(fontSize: 12))),
