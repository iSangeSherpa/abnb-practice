...
class VerifyOTPCodeViewModel : ViewModel{
    ...
    func resendEmailVerificationToken(){
        userRepository
            .resendEmailVerificationToken(email: SessionManager.shared.email ?? "")
    ...
    func verifyEmail(){
        ...
        userRepository
            .verifyEmail(email: SessionManager.shared.email!, token: getOtpCode())
            .observe(on: MainScheduler.instance)
            .flatMap{ [unowned self] it in
                self.userRepository.login(
                    email: SessionManager.shared.email ?? "",
                    password: password
                )
            }
            .do(
